...
For my project, I used a neural network on Jetson-Inference called Image-net. Image-net lets you use the AI to scan images and uses images classification to determine what each object is. For Image-net, what you have to do is train the model to recognize certain things that you want. For me, I did it to recognize if whether apples, bananas, and oranges are ripe, unripe, or rotten. To train the model, I ran a command in the docker of Jetson-Inference to prompt the training. This was the command: python3 train.py --model-dir=models/dataset data/dataset. This made the model run through 35 different epochs of every single image in the folder which trained it to recognize whether fruits are unripe, ripe, or rotten
...
To use my model, you must:
    ...
    5. Run this command: imagenet.py --model=$NET/resnet18.onnx --input_blob=input_0 --output_blob=output_0 --labels=$DATASET/labels.txt $DATASET/test/TestHere/Test.png  TestPhoto.jpg

    6. Click your image to see results!

2. Things Downloaded for this Project
